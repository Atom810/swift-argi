...
# TODO: merge to .utils
from .utils.protocol import RegisterRequest, ChatRequest, ChatResponse, DialogsResponse, DialogMessagesResponse
from swift.database.auth import Token, create_access_token, token_verify
from swift.database.record import User, Dialog, Message
from swift.database.utils import authenticate_user, register_user, create_dialog, select_dialog, find_message_history, insert_message, find_dialogs
from swift.database.password import get_password_hash
...
# TODO: create register request dataclass
@app.post("/register")
async def register_for_access_token(
    ...
    if not register_user(user):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Something went wrong while registering. Please try again!",
            headers={"WWW-Authenticate": "Bearer"},
        )
    # get token
    access_token_expires = timedelta(days=ACCESS_TOKEN_EXPIRE_DAYS)
...
@app.get('/v1/models')
async def get_available_models(current_user: User = Depends(token_verify)):
    global _args
...
@app.post('/v1/chat')
async def chat_async(request: ChatRequest, raw_request: Request, current_user: User = Depends(token_verify)) -> ChatResponse:
    current_time: str = str(int(time.time()))
    dialog: Dialog = Dialog(dialogid=request.dialogid, uid=current_user.uid)
    content: list = request.messages[0]["content"]
    chat_completion_request: ChatCompletionRequest = ChatCompletionRequest(
        ...
        stop=content[0].get('stop', ''),
        presence_penalty=request.presence_penalty,
        frequency_penalty=request.frequency_penalty,
    )
    
    # check if dialogid is not None
    if dialog.dialogid is not None:
        dialog = select_dialog({'dialogid': dialog.dialogid})
        if dialog is None:
            raise HTTPException(status_code=404, detail='dialog not found')
        # get the history from table messages with dialogid
        messages_history = find_message_history(dialog.dialogid)
        grouped_data = defaultdict(list)
        for item in messages_history:
            key = item[3]
            grouped_data[key].append(item)
        messages_history = [grouped_data[key] for key in sorted(grouped_data.keys())]
        for message in messages_history:
            for content_item in message:
                content_history = []
                if content_item[5] == 'text':
                    content_history.append({'type': 'text', 'text': content_item[6]})
                elif content_item[5] == 'image_url':
                    content_history.append({'type': 'image_url', 'image_url': {'url': content_item[6]}})
            chat_completion_request.messages.append({'role': message[0][4], 'content': content_history})
    else:
        dialog = create_dialog(current_user)
    
    # insert the user message into table messages
    for request_content in content:
        message_type = request_content.get('type', 'text')
        if message_type == 'image_url':
            message = request_content.get('image_url').get('url')
        else:
            message = request_content.get('text')
    ...
    chat_completion_request.messages.append({'role': 'user', 'content': content})
    chat_completion_response: ChatCompletionResponse = await inference_pt_async(chat_completion_request, raw_request)
    current_time = int(time.time())
    # insert the chat completion response into table messages
    insert_message(Message(
        dialogid=dialog.dialogid,
        uid=current_user.uid,
        timestamp=str(current_time),
        role='assistant',
        message_type='text',
        message=chat_completion_response.choices[0].message.content,
    ))
    # response's value is from the chat completion response
    chat_completion_response_attrs = {field.name: getattr(chat_completion_response, field.name) for field in fields(ChatCompletionResponse)}
    response = ChatResponse(**chat_completion_response_attrs, dialogid=str(dialog.dialogid))
    return response

@app.post('/v1/chat/dialogs')
async def check_dialogs(current_user: User = Depends(token_verify)):
    dialogs_list = find_dialogs(current_user.uid)
    dialogs_response: DialogsResponse = DialogsResponse(dialogs=[{'dialogid': str(dialog[0]), 'uid': str(dialog[1])} for dialog in dialogs_list])
    return dialogs_response

@app.post('/v1/chat/history/{dialogid}')
async def create_chat_completion(dialogid: str, current_user: User = Depends(token_verify)):
    dialog = select_dialog({'dialogid': dialogid})
    if dialog is None:
        raise HTTPException(status_code=404, detail='dialog not found')
    messages_history = find_message_history(dialog.dialogid)
    # sort by x[3] and x[0]
    messages_history.sort(key=lambda x: (x[3], x[0]))
    return DialogMessagesResponse(messages=[{'role': message[4], 'message_type': message[5], 'content': message[6]} for message in messages_history])

@app.post('/v1/chat/completions')
async def create_chat_completion(request: ChatCompletionRequest, raw_request: Request, current_user: User = Depends(token_verify)):
    global _args
...
async def create_completion(request: CompletionRequest, raw_request: Request, current_user: User = Depends(token_verify)):
    global _args
    assert _args is not None
    if request.stop is None:
        request.stop = []
